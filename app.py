...
def load_data():
    ...
    if data_path is None:
        ...
        raise FileNotFoundError(
            "chicago_community_development.geojson dataset file not found."
        )
    ...
    if "crime_per_1000_workers" not in gdf.columns:
        ...
        if (
            ...
            gdf["crime_per_1000_workers"] = (
                gdf["crime_count"]
                / gdf["labor_force"].replace(0, np.nan)
            ) * 1000
    ...
    if "development_priority_score" not in gdf.columns:
        ...
        gdf["development_priority_score"] = (
            gdf.get("unemployment_rate", 0) * 0.5
        )
...
st.sidebar.markdown(
    "Use the controls below to explore Chicago census tracts."
)
...
if selected_priority == "All":
    ...
    filtered_data = dashboard_data[
        dashboard_data["development_priority"]
        == selected_priority
    ].copy()
...
with priority2:
    ...
    fig_priority_unemployment.update_traces(
        texttemplate="%{text:.2f}%",
        textposition="outside"
    )

    fig_priority_unemployment.update_layout(
        height=480,
        autosize=True,
        showlegend=False,
        margin=dict(
...
fig_top_priority.update_traces(
    texttemplate="%{text:.2f}",
    textposition="outside"
)


fig_top_priority.update_layout(
    height=500,
    autosize=True,
    showlegend=False,
    margin=dict(
...
st.markdown(
    '<div class="section-title">'
    ' Unemployment Variation Across Chicago'
    '</div>',
    unsafe_allow_html=True
)
...
st.markdown(
    '<div class="section-title">'
    ' Relationships Between Community Conditions'
    '</div>',
    unsafe_allow_html=True
)
...
with relationship2:
    ...
    education_data = filtered_data.dropna(
        subset=[
            "unemployment_rate",
            "college_education_rate"
        ]
...
with st.expander(" Dashboard Guide"):
    ...
    **1. Map Indicator**
    ...
    Select an indicator to change the variable displayed
    on the Chicago census tract map.

    **2. Development Priority**

    Filter the dashboard to focus on Low, Moderate, High,
    or Very High Priority areas.

    **3. Key Performance Indicators**

    The KPI cards provide a quick summary of the selected
    census tracts. Hover over a card to see what each
    ...
    **5. Development Priority**
    ...
    Higher development priority scores identify census
    tracts with greater combinations of community challenges.

    **6. Data Table**

    The table provides the underlying census tract-level
